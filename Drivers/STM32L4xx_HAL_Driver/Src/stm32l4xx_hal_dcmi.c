...
HAL_StatusTypeDef HAL_DCMI_Init(DCMI_HandleTypeDef* hdcmi)
{
  ...
  hdcmi->Instance->CR |= (uint32_t)(hdcmi->Init.SynchroMode | hdcmi->Init.CaptureRate | hdcmi->Init.VSPolarity |
...
HAL_StatusTypeDef HAL_DCMI_Start_DMA(DCMI_HandleTypeDef* hdcmi, uint32_t DCMI_Mode, uint32_t pData, uint32_t Length)
{
  ...
  else /* Capture length is longer than DMA maximum transfer size */
  {
    ...
    if (HAL_DMA_Start_IT(hdcmi->DMA_Handle, (uint32_t)&hdcmi->Instance->DR, (uint32_t)hdcmi->pCircularBuffer,
                         circular_copy_length) != HAL_OK)
    {
      /* Update error code */
      hdcmi->ErrorCode |= HAL_DCMI_ERROR_DMA;
                                      ...
                                      uint32_t YSize)
{
  ...
  MODIFY_REG(hdcmi->Instance->CWSIZER, (DCMI_CWSIZE_VLINE | DCMI_CWSIZE_CAPCNT),
             (XSize | (YSize << DCMI_CWSIZE_VLINE_Pos)));
...
static void DCMI_DMAXferCplt(DMA_HandleTypeDef* hdma)
{
  ...
  if (hdcmi->XferCount != 0U)
  {
    ...
    else
    {
      ...
      /* Enable the DMA Channel */
      if (HAL_DMA_Start_IT(hdcmi->DMAM2M_Handle, (uint32_t)tmpBuffer_Orig, (uint32_t)tmpBuffer_Dest, loop_length) !=
          HAL_OK)
      {
        /* Update error code */
        hdcmi->ErrorCode |= HAL_DCMI_ERROR_DMA;
...
static uint32_t DCMI_TransferSize(uint32_t InputSize)
{
  ...
  static const uint32_t PrimeArray[NPRIME] = { 1UL,  2UL,  3UL,  5UL,  7UL,  11UL, 13UL, 17UL,
                                               19UL, 23UL, 29UL, 31UL, 37UL, 41UL, 43UL, 47UL };
