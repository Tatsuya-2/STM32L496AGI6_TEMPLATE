...
#define DCMI_MODE_CONTINUOUS                                                                                           \
  ((uint32_t)0x00000000U) /*!< The received data are transferred continuously                                          \
                               into the destination memory through the DMA             */
#define DCMI_MODE_SNAPSHOT                                                                                             \
  ((uint32_t)DCMI_CR_CM) /*!< Once activated, the interface waits for the start of                                     \
...
#define DCMI_SYNCHRO_HARDWARE                                                                                          \
  ((uint32_t)0x00000000U) /*!< Hardware synchronization data capture (frame/line start/stop)                           \
                               is synchronized with the HSYNC/VSYNC signals                  */
#define DCMI_SYNCHRO_EMBEDDED                                                                                          \
  ((uint32_t)DCMI_CR_ESS) /*!< Embedded synchronization data capture is synchronized with                              \
...
#define DCMI_EXTEND_DATA_14B                                                                                           \
  ((uint32_t)(DCMI_CR_EDM_0 | DCMI_CR_EDM_1)) /*!< Interface captures 14-bit data on every pixel clock */
...
#define DCMI_OEBS_ODD                                                                                                  \
  ((uint32_t)0x00000000U) /*!< Interface captures first data from the frame/line start, second one being dropped */
#define DCMI_OEBS_EVEN                                                                                                 \
  ((uint32_t)DCMI_CR_OEBS) /*!< Interface captures second data from the frame/line start, first one being dropped */
...
#define DCMI_OELS_ODD                                                                                                  \
  ((uint32_t)0x00000000U) /*!< Interface captures first line from the frame start, second one being dropped */
#define DCMI_OELS_EVEN                                                                                                 \
  ((uint32_t)DCMI_CR_OELS) /*!< Interface captures second line from the frame start, first one being dropped */
...
#define DCMI_FLAG_HSYNC                                                                                                \
  ((uint32_t)DCMI_SR_INDEX | DCMI_SR_HSYNC) /*!< HSYNC pin state (active line / synchronization between lines)   */
#define DCMI_FLAG_VSYNC                                                                                                \
  ((uint32_t)DCMI_SR_INDEX | DCMI_SR_VSYNC) /*!< VSYNC pin state (active frame / synchronization between frames) */
#define DCMI_FLAG_FNE                                                                                                  \
  ((uint32_t)DCMI_SR_INDEX | DCMI_SR_FNE) /*!< FIFO not empty flag                                                 */
/**
...
#define DCMI_FLAG_FRAMEMI                                                                                              \
  ((uint32_t)DCMI_MIS_INDEX | DCMI_MIS_FRAME_MIS) /*!< DCMI Capture complete masked interrupt status      */
#define DCMI_FLAG_OVRMI                                                                                                \
  ((uint32_t)DCMI_MIS_INDEX | DCMI_MIS_OVR_MIS) /*!< DCMI Overrun masked interrupt status               */
#define DCMI_FLAG_ERRMI                                                                                                \
  ((uint32_t)DCMI_MIS_INDEX | DCMI_MIS_ERR_MIS) /*!< DCMI Synchronization error masked interrupt status */
#define DCMI_FLAG_VSYNCMI                                                                                              \
  ((uint32_t)DCMI_MIS_INDEX | DCMI_MIS_VSYNC_MIS) /*!< DCMI VSYNC masked interrupt status                 */
#define DCMI_FLAG_LINEMI                                                                                               \
  ((uint32_t)DCMI_MIS_INDEX | DCMI_MIS_LINE_MIS) /*!< DCMI Line masked interrupt status                  */
/**
 * @}
 */
...
#define __HAL_DCMI_GET_FLAG(__HANDLE__, __FLAG__)                                                                      \
  ((((__FLAG__) & (DCMI_SR_INDEX | DCMI_MIS_INDEX)) == 0x0U) ? ((__HANDLE__)->Instance->RISR & (__FLAG__)) :           \
   (((__FLAG__)&DCMI_SR_INDEX) == 0x0U)                      ? ((__HANDLE__)->Instance->MISR & (__FLAG__)) :           \
...
#define IS_DCMI_SYNCHRO(MODE) (((MODE) == DCMI_SYNCHRO_HARDWARE) || ((MODE) == DCMI_SYNCHRO_EMBEDDED))

#define IS_DCMI_PCKPOLARITY(POLARITY)                                                                                  \
  (((POLARITY) == DCMI_PCKPOLARITY_FALLING) || ((POLARITY) == DCMI_PCKPOLARITY_RISING))
...
#define IS_DCMI_MODE_JPEG(JPEG_MODE) (((JPEG_MODE) == DCMI_JPEG_DISABLE) || ((JPEG_MODE) == DCMI_JPEG_ENABLE))

#define IS_DCMI_CAPTURE_RATE(RATE)                                                                                     \
  (((RATE) == DCMI_CR_ALL_FRAME) || ((RATE) == DCMI_CR_ALTERNATE_2_FRAME) || ((RATE) == DCMI_CR_ALTERNATE_4_FRAME))

#define IS_DCMI_EXTENDED_DATA(DATA)                                                                                    \
  (((DATA) == DCMI_EXTEND_DATA_8B) || ((DATA) == DCMI_EXTEND_DATA_10B) || ((DATA) == DCMI_EXTEND_DATA_12B) ||          \
...
#define IS_DCMI_BYTE_SELECT_MODE(MODE)                                                                                 \
  (((MODE) == DCMI_BSM_ALL) || ((MODE) == DCMI_BSM_OTHER) || ((MODE) == DCMI_BSM_ALTERNATE_4) ||                       \
