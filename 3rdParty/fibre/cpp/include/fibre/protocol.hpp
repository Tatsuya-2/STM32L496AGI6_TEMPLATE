...
constexpr uint16_t RX_BUF_SIZE =
    128;  // larger values than 128 have currently no effect because of protocol limitations
...
template <typename T>
typename std::enable_if_t<std::is_const<T>::value, size_t> write_le(T value, uint8_t* buffer)
{
...
{
public:
  StreamToPacketSegmenter(PacketSink& output) : output_(output){};
...
{
public:
  StreamBasedPacketSink(StreamSink& output) : output_(output){};
...
{
public:
  MemoryStreamSink(uint8_t* buffer, size_t length) : buffer_(buffer), buffer_length_(length)
  {
  }
...
{
public:
  NullStreamSink(size_t skip, StreamSink& follow_up_stream) : skip_(skip), follow_up_stream_(follow_up_stream)
  {
  }
...
template <typename T>
bool default_readwrite_endpoint_handler(endpoint_ref_t* value, const uint8_t* input, size_t input_length,
                                        StreamSink* output)
{
...
{
public:
  BidirectionalPacketBasedChannel(PacketSink& output) : output_(output)
  {
  }
...
public:
  ...
  Endpoint* get_by_name(const char* name, size_t length)
  {
    ...
    if (result)
      ...
      return subsequent_members_.get_by_name(name, length);
  }
...
public:
  ProtocolObject(const char* name, TMembers&&... member_list)
    : name_(name), member_list_(std::forward<TMembers>(member_list)...)
  {
  }
  ...
  void write_json(size_t id, StreamSink* output)
  {
    ...
    member_list_.write_json(id, output), write_string("]}", output);
  }
...
public:
  ...
  ProtocolProperty(const char* name, TProperty* property, void (*written_hook)(void*), void* ctx)
...
ProtocolProperty<const std::underlying_type_t<TProperty>> make_protocol_ro_property(
    const char* name, TProperty* property, void (*written_hook)(void*) = nullptr, void* ctx = nullptr)
{
...
struct PropertyListFactory<>
{
  ...
  static MemberList<> make_property_list(std::array<const char*, sizeof...(TAllProperties)> names,
...
public:
  ...
  void write_json(size_t id, StreamSink* output)
  {
    ...
    input_properties_.write_json(id + 1, output), write_string("],\"outputs\":[", output);
...
ProtocolFunction<TObj, std::tuple<TArgs...>, std::tuple<>> make_protocol_function(const char* name, TObj& obj,
...
ProtocolFunction<TObj, std::tuple<TArgs...>, std::tuple<TRet>> make_protocol_function(const char* name, TObj& obj,
                                                                                      TRet (TObj::*func_ptr)(TArgs...),
                                                                                      TNames... names)
{
  return ProtocolFunction<TObj, std::tuple<TArgs...>, std::tuple<TRet>>(name, obj, func_ptr, { names... },
                                                                        { "result" });
}
